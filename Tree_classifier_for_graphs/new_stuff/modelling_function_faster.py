...
def log_print(*args, **kwargs):
    ...
    # Also print to ensure it appears in terminal immediately
...
def intra_bayesian_optimization(
    ...
    *,
    ...
    for fold, (y_test, y_scores, auc) in enumerate(fold_results, start=1):
        roc_aucs.append(auc)
        logging.info(f"Fold {fold} ROC AUC: {auc:.3f}")

        # Optimized threshold computation using vectorized operations
        y_test_bool = y_test.astype(bool)
        for i, t in enumerate(thresholds):
            y_pred = (y_scores >= t).astype(int)
            # Fast vectorized metric computation
...
def calculate_global_shap_values(dfs, feature_columns, best_params, target_loop=6, 
    ...
    """
    Calculate global SHAP values for the model (optimized with sampling).
    ...
    Args:
        ...
        shap_sample_size: If provided, only compute SHAP for a sample (faster)
    ...
    # OPTIMIZATION: Only compute SHAP for a sample if specified (much faster for large datasets)
    if shap_sample_size is not None and X_scaled.shape[0] > shap_sample_size:
        np.random.seed(random_state)
        sample_indices = np.random.choice(X_scaled.shape[0], size=shap_sample_size, replace=False)
        X_sample = X_scaled[sample_indices]
        shap_values_sample = explainer.shap_values(X_sample)
        
        # For compatibility, create full-size array with NaN or zeros for non-sampled
        # But return the sampled version for actual importance calculation
        shap_values = shap_values_sample
        X_scaled_used = X_sample
    else:
        # Calculate SHAP values for all samples
        shap_values = explainer.shap_values(X_scaled)
        X_scaled_used = X_scaled
    
    return shap_values, explainer, model, X_scaled_used
...
def run_bayes_shap_for_loop(
    ...
    *,
    ...
    for colset_name, feature_cols in column_defs.items():
        ...
        # OPTIMIZATION: Use shap_sample_size for faster SHAP computation
        ...
        # SHAP summary plot (using already sampled/limited data if shap_sample_size was used)
    ...
    # Log completion and log file location
    logging.info("\n" + "="*80)
