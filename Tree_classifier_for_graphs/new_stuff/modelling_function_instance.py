...
# =================== CORE: BAYESIAN OPTIMIZATION (STABILIZED) ===================
def intra_bayesian_optimization(
    ...
    *,
    n_calls=20,
    n_splits=5,
    n_workers=1,                 # parallel across folds (processes)
    results_dir="bayes_results",
    save_plot=True,
    random_state=42
):
    """
    Parallel CV with process-based isolation and memory-safe XGBoost settings.
    """
    ...
    dimensions = [
        Integer(50, 1000, name='n_estimators'),   # restored
    ...
    fold_jobs = max(1, min(int(n_workers), n_splits))
    ...
    def objective(**params):
        try:
            # Context-managed loky pool + dedicated temp dir
    ...
    # Context-managed pool for final evaluation
    with parallel_backend("loky", inner_max_num_threads=1):
        with Parallel(
            n_jobs=fold_jobs,
            backend="loky",
...
def run_bayes_shap_for_loop(
    ...
    *,
    ...
    n_calls=20,
    n_splits=5,
    n_workers=8,                   # will be capped to n_splits
    random_state=42,
    ...
    logging.info(f"  n_workers: {n_workers} (will cap to {min(n_workers, n_splits)})")
    ...
    def _oof_scores_with_best_params(dfs, feature_cols, loop, best_params, *,
                                     n_splits=5, n_workers=8, random_state=42):
        """Out-of-fold predictions (no leakage) with safe settings."""
        data = dfs[dfs['loops'] == loop]
    ...
    for colset_name, feature_cols in column_defs.items():
        ...
        results = intra_bayesian_optimization(
            ...
            n_calls=20,
            n_splits=5,
            n_workers=min(n_workers, n_splits),  # cap to folds
            results_dir="bayes_results",
        ...
        y_true_oof, y_score_oof = _oof_scores_with_best_params(
            ...
            n_splits=n_splits, n_workers=min(n_workers, n_splits), random_state=random_state
        )
