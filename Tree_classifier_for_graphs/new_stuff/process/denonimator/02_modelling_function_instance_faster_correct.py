...
# =================== CORE: BAYESIAN OPTIMIZATION WITH 80/20 SPLIT ===================
def intra_bayesian_optimization(
    ...
    *,
    ...
    Uses a stratified 80/20 train/hold-out split. CV + Bayesian optimization
    are done on the 80% train subset; we then train on that 80% and evaluate
    once on the 20% hold-out, producing a second AUC.
    """
    ...
    # CV is now only over the training portion (X_cv, y_cv)
    ...
    # Sequential final evaluation on train subset
    with parallel_backend("loky", inner_max_num_threads=1):
        with Parallel(
    ...
    # Save threshold sweep CSV (same filename/path as before)
    thr_df = pd.DataFrame({
    ...
    # ----------------------- OOF AUC on 80% training split -----------------------
    # Compute OOF predictions on the 80% training split (concatenated from all folds)
    logging.info("\n" + "="*60)
    ...
    # Sequential OOF on training split
    with parallel_backend("loky", inner_max_num_threads=1):
        with Parallel(
            n_jobs=1,  # force sequential
            backend="loky",
    ...
    y_true_oof_train = np.concatenate([yt for yt, _, _ in fold_out])
    y_score_oof_train = np.concatenate([ys for _, ys, _ in fold_out])
    fold_aucs_train = [auc_fold for _, _, auc_fold in fold_out]
    oof_auc_train = float(roc_auc_score(y_true_oof_train, y_score_oof_train))
    oof_auc_train_mean = float(np.mean(fold_aucs_train))
    logging.info(f"OOF AUC (80% training split): {oof_auc_train:.4f} (single), {oof_auc_train_mean:.4f} (mean over folds)")
    ...
    summary = {
        ...
        "cv_auc_std": std_auc,
        "oof_auc_train": oof_auc_train,   # OOF AUC on 80% training split (single from concatenated)
        "oof_auc_train_mean": oof_auc_train_mean,  # OOF AUC on 80% training split (mean over folds)
        "holdout_auc": holdout_auc,       # NEW: AUC on 20% hold-out set
        "paths": {
...
def run_bayes_shap_for_loop(
    ...
    *,
    ...
    Uses intra_bayesian_optimization with an internal 80/20
    stratified train/hold-out split.
    """
    # ---------------------------
    # Setup logging
    # ---------------------------
    ...
    def _oof_scores_with_best_params(dfs, feature_cols, loop, best_params, *,
        ...
        """Out-of-fold predictions (no leakage inside CV), sequential, no scaling.
        Note: this uses all data for the given loop (train + hold-out) to
        generate OOF metrics and plots, as in your original pipeline.
        """
        data = dfs[dfs['loops'] == loop]
    ...
    try:
        ...
        #dfs['COEFFICIENTS'] = dfs['COEFFICIENTS'].astype(float).ne(0).astype(int)
        dfs["loops"] = i
    ...
    for colset_name, feature_cols in column_defs.items():
        ...
        # Bayesian optimization (sequential; internally does 80/20 split)
        ...
        # SHAP (no scaler; full dataset for the loop, as before)
        ...
        # OOF + curves/deciles (sequential) on full data for the loop
        ...
        # AUC summary (CV/OOF and hold-out)
        auc_summary_rows.append({
            "loop": i,
            "colset": colset_name,
            "n_features": len(feature_cols),
            "auc": curve_artifacts["auc"],                      # OOF AUC on full dataset (100%) - single from concatenated
            "auc_mean": oof_auc_mean_full,                      # OOF AUC on full dataset (100%) - mean over folds
            "oof_auc_train": results.get("oof_auc_train"),      # OOF AUC on 80% training split - single from concatenated
            "oof_auc_train_mean": results.get("oof_auc_train_mean"),  # OOF AUC on 80% training split - mean over folds
            ...
            "holdout_auc": results.get("holdout_auc")           # NEW: hold-out AUC
        })
        ...
        index_rows.append({
            ...
            "holdout_auc": results.get("holdout_auc")           # NEW
        })
