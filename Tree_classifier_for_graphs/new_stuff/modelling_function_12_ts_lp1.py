...
def bayes_opt_on_merged_loop_order(
    ...
    *,
    ...
    if cfg.test_loop is not None:
        ...
        print(f"Training final model (n_estimators={final_params['n_estimators']}) on {n_final_samples:,} samples (excluding test loop {cfg.test_loop})...", flush=True)
    else:
        X_final_train = X_all
        y_final_train = y_all
        ...
        print(f"Training final model (n_estimators={final_params['n_estimators']}) on all {n_final_samples:,} samples...", flush=True)
    ...
    logging.info(f"Training inference model with n_estimators={inference_params['n_estimators']} on {n_train_samples:,} samples (loops {unique_loops})...")
    print(f"Training inference model (n_estimators={inference_params['n_estimators']}) on {n_train_samples:,} samples (loops {unique_loops})...", flush=True)
    inference_model = XGBClassifier(
        eval_metric='logloss', random_state=cfg.random_state, n_jobs=1,
        tree_method="hist", max_bin=256, device=cfg.device, **inference_params
    )
    inference_model.fit(X_train_all, y_train_all)
