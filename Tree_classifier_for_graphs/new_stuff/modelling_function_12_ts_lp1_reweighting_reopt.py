...
# ---------- helpers ----------
def _fit_xgb_with_early_stop(model, X_train, y_train, X_val, y_val, early_rounds=50, sample_weight=None):
    """Fit XGBoost model with early stopping, optionally using sample weights."""
    try:
...
class BayesConfig:
    ...
    validation_step: int | list[int] = 1  # Step size(s) for validation (1 = l+1, 2 = l+2, etc.)
                                          # Can be a single int or list like [1, 2] to validate on both l+1 and l+2
    test_loop: int | None = None  # Optional test loop for final evaluation (e.g., 12 for loop 12)
                                   # If specified, will evaluate inference model on this loop and save scores
...
def bayes_opt_on_merged_loop_order(
    ...
    *,
    ...
    Validation splits are generated automatically from loops based on validation_step:
        If validation_step=1 (default): l+1 validation
            loops[0] -> loops[1]
            loops[0:2] -> loops[2]
            loops[0:3] -> loops[3]
            ...
        If validation_step=2: l+2 validation (skip one loop)
            loops[0] -> loops[2]
            loops[0:2] -> loops[3]
            loops[0:3] -> loops[4]
            ...
        If validation_step=[1, 2]: validate on both l+1 and l+2 simultaneously
            loops[0] -> [loops[1], loops[2]]  (average AUC)
            loops[0:2] -> [loops[2], loops[3]]  (average AUC)
            loops[0:3] -> [loops[3], loops[4]]  (average AUC)
            ...
    """
    logging.info("="*70)
    ...
    # Normalize validation_step to always be a list
    if isinstance(cfg.validation_step, int):
        validation_steps = [cfg.validation_step]
    else:
        validation_steps = sorted(list(set(cfg.validation_step)))  # Remove duplicates and sort
    
    max_step = max(validation_steps)
    if len(loops) < max_step + 1:
        raise ValueError(f"Need at least {max_step + 1} loops for validation_step={cfg.validation_step}. Got: {loops}")
    
    # Generate validation splits dynamically from loops
    # Special rule: If loop 5 is present, pack it with loop 6 (start from [5,6] -> next)
    # For loops [5, 6, 7, 8, 9, 10, 11] with validation_step=1:
    #   [5, 6] -> 7  (skip [5] -> 6)
    #   [5, 6, 7] -> 8
    #   [5, 6, 7, 8] -> 9
    #   ...
    # For validation_step=2 (l+2):
    #   [5, 6] -> 8  (skip 7)
    #   [5, 6, 7] -> 9  (skip 8)
    #   ...
    # For validation_step=[1, 2] (both l+1 and l+2):
    #   [5, 6] -> [7, 8]  (validate on both, average AUC)
    #   [5, 6, 7] -> [8, 9]  (validate on both, average AUC)
    #   ...
    ...
    if 5 in loops and len(loops) > 2:
        # Find index of 5
        idx_5 = loops.index(5)
        # If 6 comes right after 5, start from index 2 (so first split is [5,6] -> next)
    ...
    # Generate validation splits with the specified step size(s)
    # For each training set, validate on all specified steps (only if they exist)
    # Continue until we can't create any more splits
    i = start_idx
    while i < len(loops):
        train_loops = loops[:i]
        val_loops = []
        for step in validation_steps:
            val_idx = i + step - 1
            if val_idx < len(loops):
                val_loops.append(loops[val_idx])
        if val_loops:  # Only add if we have at least one validation loop
            validation_splits.append((train_loops, val_loops))
            i += 1
        else:
            # No more validation loops available, stop
            break
    
    steps_str = f"l+{'+'.join(map(str, validation_steps))}" if len(validation_steps) > 1 else f"l+{validation_steps[0]}"
    logging.info(f"Validation steps: {validation_steps} ({steps_str} validation)")
    logging.info(f"Validation splits defined: {validation_splits}")
    print(f"Validation steps: {validation_steps} ({steps_str} validation)", flush=True)
    print(f"Validation splits: {validation_splits}", flush=True)

    # === SEARCH SPACE (including sample reweighting for recall) ===
    dimensions = [
        ...
        Real(1.0, 10.0, name='pos_weight'),  # Weight multiplier for positive class (to improve recall)
    ]

    history_rows = []

    def _fit_eval_split(train_loops, val_loops, params):
        """Train on train_loops, validate on val_loops (can be single loop or list of loops)"""
        # Normalize val_loops to list
        if isinstance(val_loops, int):
            val_loops = [val_loops]
        
        train_mask = np.isin(loop_all, train_loops)
        val_mask = np.isin(loop_all, val_loops)
        ...
        if not np.any(val_mask):
            logging.warning(f"No validation data for loops {val_loops}")
            return np.nan, np.nan
        
        val_loops_str = val_loops if len(val_loops) > 1 else val_loops[0]
        logging.info(f"  Split {train_loops}->{val_loops_str}: training on {n_train:,} samples, validating on {n_val:,} samples")
        X_train, X_val = X_all[train_mask], X_all[val_mask]
        y_train, y_val = y_all[train_mask], y_all[val_mask]

        # Extract pos_weight from params and create sample weights
        xgb_params = {k: v for k, v in params.items() if k != 'pos_weight'}
        pos_weight = params.get('pos_weight', 1.0)
        
        # Create sample weights: negative class = 1.0, positive class = pos_weight
        sample_weight = np.where(y_train == 1, pos_weight, 1.0).astype(np.float32)
        
        logging.info(f"  Training model with params: {xgb_params}, pos_weight={pos_weight:.3f}")
        model = XGBClassifier(
            eval_metric='logloss',
        ...
        _fit_xgb_with_early_stop(model, X_train, y_train, X_val, y_val, early_rounds=50, sample_weight=sample_weight)
        ...
        # Compute zero removal fraction metric (to be maximized):
        # 1. Find minimum score for class 1 (positive samples)
        # 2. Count zeros with scores < min_score
        # 3. Ratio = (zeros with score < min_score) / (total zeros)
        y_val_bool = y_val.astype(bool)
        positive_scores = y_scores[y_val_bool]
        
        if len(positive_scores) > 0:
            min_positive_score = float(np.min(positive_scores))
            zero_scores = y_scores[~y_val_bool]
            n_zeros = len(zero_scores)
            if n_zeros > 0:
                zeros_below_threshold = np.sum(zero_scores < min_positive_score)
                zero_removal_fraction = zeros_below_threshold / n_zeros
            else:
                zero_removal_fraction = 0.0
                logging.warning(f"  No negative samples in validation set")
        else:
            zero_removal_fraction = 0.0
            logging.warning(f"  No positive samples in validation set, cannot compute zero removal fraction")
        
        logging.info(f"  Validation AUC: {auc_val:.4f}, Zero Removal Fraction: {zero_removal_fraction:.4f}")

        del model, X_train, X_val, y_scores, y_train, y_val
        gc.collect()
        return auc_val, zero_removal_fraction  # Return AUC as primary metric, zero removal fraction as reference
    ...
    def objective(**params):
        ...
        try:
            split_aucs = []
            split_zero_removal_fractions = []
            # Loop-order validation splits
            for idx, (train_loops, val_loops) in enumerate(validation_splits, 1):
                val_loops_str = val_loops if isinstance(val_loops, list) and len(val_loops) > 1 else (val_loops[0] if isinstance(val_loops, list) else val_loops)
                logging.info(f"  Split {idx}/{len(validation_splits)}: {train_loops} -> {val_loops_str}")
                print(f"  Split {idx}/{len(validation_splits)}: {train_loops} -> {val_loops_str}...", flush=True)
                auc, zero_removal_frac = _fit_eval_split(train_loops, val_loops, params)
                if not np.isnan(auc):
                    split_aucs.append(auc)
                    if not np.isnan(zero_removal_frac):
                        split_zero_removal_fractions.append(zero_removal_frac)
                    print(f"    AUC: {auc:.4f}, Zero Removal Fraction: {zero_removal_frac:.4f}", flush=True)
            
            if len(split_aucs) == 0:
                logging.warning(f"No valid splits for params {params}")
                history_rows.append({**params, "mean_auc": np.nan, "std_auc": np.nan, "mean_zero_removal_fraction": np.nan, "std_zero_removal_fraction": np.nan, "error": "No valid splits", "ts": time.time()})
                return 1.0
            
            mean_auc = float(np.mean(split_aucs))
            std_auc = float(np.std(split_aucs)) if len(split_aucs) > 1 else 0.0
            mean_zero_removal_fraction = float(np.mean(split_zero_removal_fractions)) if split_zero_removal_fractions else np.nan
            std_zero_removal_fraction = float(np.std(split_zero_removal_fractions)) if len(split_zero_removal_fractions) > 1 else 0.0
            elapsed = time.time() - start_time
            logging.info(f"[ITER {iter_num}] Mean AUC: {mean_auc:.4f} ± {std_auc:.4f}, Mean Zero Removal Fraction: {mean_zero_removal_fraction:.4f} ± {std_zero_removal_fraction:.4f} (took {elapsed:.1f}s)")
            print(f"[ITER {iter_num}] Mean AUC: {mean_auc:.4f} ± {std_auc:.4f}, Mean Zero Removal Fraction: {mean_zero_removal_fraction:.4f} ± {std_zero_removal_fraction:.4f} (took {elapsed:.1f}s)", flush=True)
            history_rows.append({**params, "mean_auc": mean_auc, "std_auc": std_auc, "mean_zero_removal_fraction": mean_zero_removal_fraction, "std_zero_removal_fraction": std_zero_removal_fraction, "ts": time.time()})
            return -mean_auc  # Negative because gp_minimize minimizes, we want to maximize AUC
        except Exception as e:
            elapsed = time.time() - start_time
            ...
            history_rows.append({**params, "mean_zero_removal_fraction": np.nan, "std_zero_removal_fraction": np.nan, "mean_auc": np.nan, "std_auc": np.nan, "error": str(e), "ts": time.time()})
            return 1.0

    logging.info("\n" + "="*70)
    logging.info("Starting LOOP-ORDER Bayesian Hyperparameter Optimization (optimizing for AUC ROC)")
    splits_str = ", ".join([f"{t}->{v if isinstance(v, list) and len(v) > 1 else (v[0] if isinstance(v, list) else v)}" for t, v in validation_splits])
    logging.info(f"Validation splits: {splits_str}")
    logging.info(f"Total iterations: {cfg.n_calls}")
    logging.info("=" * 70)
    print(f"\nStarting Bayesian optimization ({cfg.n_calls} iterations) - optimizing for AUC ROC...", flush=True)
    ...
    best_score = float(-result.fun)  # This is the AUC (we return -mean_auc)
    logging.info("\n" + "="*70)
    ...
    for idx, (train_loops, val_loops) in enumerate(validation_splits, 1):
        ...
        val_loops_str = val_loops if len(val_loops) > 1 else val_loops[0]
        logging.info(f"\nProcessing split {idx}/{len(validation_splits)}: {train_loops} -> {val_loops_str}")
        print(f"  Split {idx}/{len(validation_splits)}: {train_loops} -> {val_loops_str}...", flush=True)
        ...
        if not np.any(train_mask) or not np.any(val_mask):
            logging.warning(f"Skipping split {train_loops}->{val_loops_str} (missing data)")
        ...
        # Extract pos_weight from best_params and create sample weights
        xgb_params = {k: v for k, v in best_params.items() if k != 'pos_weight'}
        pos_weight = best_params.get('pos_weight', 1.0)
        sample_weight = np.where(y_tr == 1, pos_weight, 1.0).astype(np.float32)
        
        logging.info(f"  Training model with best parameters (pos_weight={pos_weight:.3f})...")
        print(f"    Training model (pos_weight={pos_weight:.3f})...", flush=True)
        model = XGBClassifier(
            eval_metric='logloss',
            random_state=cfg.random_state,
            n_jobs=1,
        ...
        _fit_xgb_with_early_stop(model, X_tr, y_tr, X_te, y_te, early_rounds=50, sample_weight=sample_weight)
        ...
        logging.info(f"  Split {train_loops}->{val_loops_str}: AUC = {auc_val:.4f}")
        print(f"    AUC: {auc_val:.4f}", flush=True)
        split_results.append({
            "train_loops": train_loops,
            "val_loops": val_loops if len(val_loops) > 1 else val_loops[0],
            "auc": float(auc_val),
            "n_train": int(n_train),
            "n_val": int(n_val)
        })
    ...
    # Compute zero removal fraction from OOF predictions
    oof_mask = (oof_scores != 0) | (np.array(oof_true) != 0)
    if np.any(oof_mask):
        oof_scores_masked = np.array(oof_scores)[oof_mask]
        oof_true_masked = np.array(oof_true)[oof_mask]
        oof_true_bool = oof_true_masked.astype(bool)
        positive_scores_oof = oof_scores_masked[oof_true_bool]
        if len(positive_scores_oof) > 0:
            min_positive_score_oof = float(np.min(positive_scores_oof))
            zero_scores_oof = oof_scores_masked[~oof_true_bool]
            n_zeros_oof = len(zero_scores_oof)
            if n_zeros_oof > 0:
                zeros_below_threshold_oof = np.sum(zero_scores_oof < min_positive_score_oof)
                cv_zero_removal_fraction = zeros_below_threshold_oof / n_zeros_oof
            else:
                cv_zero_removal_fraction = 0.0
        else:
            cv_zero_removal_fraction = 0.0
    else:
        cv_zero_removal_fraction = 0.0
    
    logging.info(f"\nOverall CV Zero Removal Fraction: {cv_zero_removal_fraction:.4f}")
    logging.info(f"Overall CV ROC AUC: {mean_auc:.3f} ± {std_auc:.3f}")
    print(f"\nOverall CV Zero Removal Fraction: {cv_zero_removal_fraction:.4f}", flush=True)
    print(f"Overall CV ROC AUC: {mean_auc:.3f} ± {std_auc:.3f}", flush=True)
    ...
    # Extract pos_weight from best_params (it's not an XGBoost parameter)
    xgb_best_params = {k: v for k, v in best_params.items() if k != 'pos_weight'}
    pos_weight = best_params.get('pos_weight', 1.0)
    final_params = dict(xgb_best_params)  # Only XGBoost parameters
    ...
    model_tmp = XGBClassifier(
        ...
        tree_method="hist", max_bin=256, device=cfg.device, **xgb_best_params
    )
    # Use last split for determining best iteration
    if validation_splits:
        last_train_loops, last_val_loops = validation_splits[-1]
        # Normalize to list
        if isinstance(last_val_loops, int):
            last_val_loops = [last_val_loops]
        train_mask = np.isin(loop_all, last_train_loops)
        val_mask = np.isin(loop_all, last_val_loops)
        # Use first validation loop for early stopping
        last_val_loop = last_val_loops[0]
    else:
    ...
    if np.any(train_mask) and np.any(val_mask):
        ...
        sample_weight_tmp = np.where(y_tr == 1, pos_weight, 1.0).astype(np.float32)
        _fit_xgb_with_early_stop(model_tmp, X_tr, y_tr, X_val, y_val, early_rounds=50, sample_weight=sample_weight_tmp)
        best_iter = getattr(model_tmp, "best_iteration", None)
        if best_iter is None: best_iter = xgb_best_params["n_estimators"] - 1
        ...
        best_iter = xgb_best_params["n_estimators"] - 1
        logging.info(f"Using default n_estimators: {best_iter + 1}")
    gc.collect()

    final_params["n_estimators"] = int(best_iter + 1)
    
    # Exclude test loop from final model training if specified
    if cfg.test_loop is not None:
        final_train_mask = (loop_all != cfg.test_loop)
        X_final_train = X_all[final_train_mask]
        y_final_train = y_all[final_train_mask]
        n_final_samples = X_final_train.shape[0]
        logging.info(f"Training final model with n_estimators={final_params['n_estimators']} on {n_final_samples:,} samples (excluding test loop {cfg.test_loop})...")
        print(f"Training final model (n_estimators={final_params['n_estimators']}, pos_weight={pos_weight:.3f}) on {n_final_samples:,} samples (excluding test loop {cfg.test_loop})...", flush=True)
    else:
        X_final_train = X_all
        y_final_train = y_all
        n_final_samples = X_all.shape[0]
        logging.info(f"Training final model with n_estimators={final_params['n_estimators']} on all {n_final_samples:,} samples...")
        print(f"Training final model (n_estimators={final_params['n_estimators']}, pos_weight={pos_weight:.3f}) on all {n_final_samples:,} samples...", flush=True)
    
    # Create sample weights for final model
    final_sample_weight = np.where(y_final_train == 1, pos_weight, 1.0).astype(np.float32)
    
    final_model = XGBClassifier(
        eval_metric='logloss', random_state=cfg.random_state, n_jobs=1,
        tree_method="hist", max_bin=256, device=cfg.device, **final_params
    )
    final_model.fit(X_final_train, y_final_train, sample_weight=final_sample_weight)
    ...
    # Use only the training loops (exclude test loop if specified)
    # The test loop should NOT be used for training
    unique_loops = loops  # Use only the loops specified for training
    if cfg.test_loop is not None:
        logging.info(f"Excluding test loop {cfg.test_loop} from training")
        print(f"Excluding test loop {cfg.test_loop} from training", flush=True)
    
    # Create mask to exclude test loop from training
    if cfg.test_loop is not None:
        inference_train_mask = (loop_all != cfg.test_loop)
        X_train_all = X_all[inference_train_mask]
        y_train_all = y_all[inference_train_mask]
        n_train_samples = X_train_all.shape[0]
    else:
        inference_train_mask = np.ones(X_all.shape[0], dtype=bool)
        X_train_all = X_all
        y_train_all = y_all
        n_train_samples = X_all.shape[0]
    
    logging.info(f"Training on loops: {unique_loops}")
    logging.info(f"Total training samples: {n_train_samples:,}")
    print(f"Training on loops: {unique_loops}", flush=True)
    print(f"Total training samples: {n_train_samples:,}", flush=True)
    
    # Use the best parameters from optimization (excluding pos_weight from XGB params)
    inference_params = {k: v for k, v in best_params.items() if k != 'pos_weight'}
    pos_weight_inf = best_params.get('pos_weight', 1.0)
    ...
    if np.any(train_mask) and np.any(val_mask):
        ...
        X_tr, X_val = X_all[train_mask], X_all[val_mask]
        y_tr, y_val = y_all[train_mask], y_all[val_mask]
        sample_weight_tmp = np.where(y_tr == 1, pos_weight_inf, 1.0).astype(np.float32)
        _fit_xgb_with_early_stop(model_tmp, X_tr, y_tr, X_val, y_val, early_rounds=50, sample_weight=sample_weight_tmp)
        best_iter = getattr(model_tmp, "best_iteration", None)
        if best_iter is None: best_iter = inference_params["n_estimators"] - 1
        logging.info(f"Best iteration determined: {best_iter + 1}")
        print(f"Best iteration: {best_iter + 1}", flush=True)
        del model_tmp, X_tr, X_val, y_tr, y_val
    else:
        best_iter = inference_params["n_estimators"] - 1
        logging.info(f"Using default n_estimators: {best_iter + 1}")
    gc.collect()
    
    inference_params["n_estimators"] = int(best_iter + 1)
    
    # Create sample weights for inference model
    inference_sample_weight = np.where(y_train_all == 1, pos_weight_inf, 1.0).astype(np.float32)
    
    # Train on training loops only (exclude test loop)
    logging.info(f"Training inference model with n_estimators={inference_params['n_estimators']}, pos_weight={pos_weight_inf:.3f} on {n_train_samples:,} samples (loops {unique_loops})...")
    print(f"Training inference model (n_estimators={inference_params['n_estimators']}, pos_weight={pos_weight_inf:.3f}) on {n_train_samples:,} samples (loops {unique_loops})...", flush=True)
    inference_model = XGBClassifier(
        eval_metric='logloss', random_state=cfg.random_state, n_jobs=1,
        tree_method="hist", max_bin=256, device=cfg.device, **inference_params
    )
    inference_model.fit(X_train_all, y_train_all, sample_weight=inference_sample_weight)
    ...
    test_loop_note = f"Test loop {cfg.test_loop} was excluded from training and used only for evaluation.\n" if cfg.test_loop is not None else ""
    with open(run_dir / "inference_readme.txt", "w") as f:
        f.write(
            ...
            f"Total training samples: {n_train_samples:,}\n"
            f"{test_loop_note}"
    ...
    # ---------- TEST LOOP EVALUATION (if specified) ----------
    test_auc = None
    test_zero_removal_fraction = None
    test_scores_path = None
    if cfg.test_loop is not None:
        logging.info("\n" + "="*70)
        logging.info(f"Evaluating inference model on test loop {cfg.test_loop}")
        logging.info("="*70)
        print("\n" + "="*70, flush=True)
        print(f"Evaluating inference model on test loop {cfg.test_loop}", flush=True)
        print("="*70, flush=True)
        
        # Check if test loop exists in data
        test_mask = (loop_all == cfg.test_loop)
        n_test = np.sum(test_mask)
        
        if not np.any(test_mask):
            logging.warning(f"Test loop {cfg.test_loop} not found in data. Available loops: {sorted(np.unique(loop_all).tolist())}")
            print(f"WARNING: Test loop {cfg.test_loop} not found in data", flush=True)
        else:
            X_test = X_all[test_mask]
            y_test = y_all[test_mask]
            
            logging.info(f"Test set: {n_test:,} samples from loop {cfg.test_loop}")
            print(f"Test set: {n_test:,} samples from loop {cfg.test_loop}", flush=True)
            
            # Get predictions from inference model
            logging.info("Computing predictions on test set...")
            print("Computing predictions on test set...", flush=True)
            test_scores = inference_model.predict_proba(X_test)[:, 1].astype(np.float32)
            
            # Compute AUC
            test_auc = roc_auc_score(y_test, test_scores)
            logging.info(f"Test AUC (loop {cfg.test_loop}): {test_auc:.4f}")
            print(f"Test AUC (loop {cfg.test_loop}): {test_auc:.4f}", flush=True)
            
            # Compute zero removal fraction for test set
            y_test_bool = y_test.astype(bool)
            positive_scores_test = test_scores[y_test_bool]
            
            if len(positive_scores_test) > 0:
                min_positive_score_test = float(np.min(positive_scores_test))
                zero_scores_test = test_scores[~y_test_bool]
                n_zeros_test = len(zero_scores_test)
                if n_zeros_test > 0:
                    zeros_below_threshold_test = np.sum(zero_scores_test < min_positive_score_test)
                    test_zero_removal_fraction = zeros_below_threshold_test / n_zeros_test
                else:
                    test_zero_removal_fraction = 0.0
                    logging.warning("No negative samples in test set")
            else:
                test_zero_removal_fraction = 0.0
                logging.warning("No positive samples in test set, cannot compute zero removal fraction")
            
            logging.info(f"Test Zero Removal Fraction (loop {cfg.test_loop}): {test_zero_removal_fraction:.4f}")
            print(f"Test Zero Removal Fraction (loop {cfg.test_loop}): {test_zero_removal_fraction:.4f}", flush=True)
            
            # Save test AUC and zero removal fraction to file
            test_auc_path = run_dir / f"test_loop_{cfg.test_loop}_auc.txt"
            with open(test_auc_path, "w") as f:
                f.write(f"Test Loop: {cfg.test_loop}\n")
                f.write(f"Test AUC: {test_auc:.6f}\n")
                f.write(f"Test Zero Removal Fraction: {test_zero_removal_fraction:.6f}\n")
                f.write(f"Number of test samples: {n_test:,}\n")
            logging.info(f"Test metrics saved to: {test_auc_path}")
            print(f"Test metrics saved to: {test_auc_path}", flush=True)
            ...
            test_scores_path = run_dir / f"test_loop_{cfg.test_loop}_scores.csv"
            test_scores_df.to_csv(test_scores_path, index=False)
            logging.info(f"Test scores saved to: {test_scores_path}")
            print(f"Test scores saved to: {test_scores_path}", flush=True)
            
            # Log class distribution in test set
            test_class_dist = dict(zip(*np.unique(y_test, return_counts=True)))
            logging.info(f"Test set class distribution: {test_class_dist}")
            print(f"Test set class distribution: {test_class_dist}", flush=True)
    ...
    summary = {
        ...
        "cv_auc_std": std_auc,
        "cv_zero_removal_fraction": float(cv_zero_removal_fraction),
        ...
        "inference_model_total_samples": int(n_train_samples),
        "test_loop": cfg.test_loop,
        "test_auc": float(test_auc) if test_auc is not None else None,
        "test_zero_removal_fraction": float(test_zero_removal_fraction) if test_zero_removal_fraction is not None else None,
        "paths": {
            ...
            "test_scores_csv": str(test_scores_path.resolve()) if test_scores_path else None,
            "test_auc_txt": str((run_dir / f"test_loop_{cfg.test_loop}_auc.txt").resolve()) if (cfg.test_loop is not None and test_auc is not None) else None,
        }
    }
    ...
    return {
        ...
        'cv_auc_mean': mean_auc,
        'cv_auc_std': std_auc,
        'cv_zero_removal_fraction': cv_zero_removal_fraction,
        ...
        'test_auc': test_auc,
        'test_zero_removal_fraction': test_zero_removal_fraction,
        'test_scores_path': str(test_scores_path.resolve()) if test_scores_path else None
    }
...
def run_bayes_on_merged_parquet(
    ...
    # Include test loop in data loading if specified (so it's available for evaluation)
    loops_to_load = set(loops_list)
    if cfg.test_loop is not None:
        loops_to_load.add(cfg.test_loop)
        logging.info(f"Test loop {cfg.test_loop} will be included in data loading for evaluation")
    loops_to_load = sorted(list(loops_to_load))
    
    logging.info(f"Requested training loops: {loops_list}")
    logging.info(f"Loops to load from data: {loops_to_load}")
    
    merged = load_and_align_parquet_dataset_lean(dataset_dir, loops=loops_to_load)
    ...
    logging.info(f"CV AUC (OOF): {out['cv_auc_mean']:.4f} ± {out['cv_auc_std']:.4f}")
    logging.info(f"CV Zero Removal Fraction (OOF): {out['cv_zero_removal_fraction']:.4f}")
...
if __name__ == "__main__":
    ...
    LOOPS_TO_USE = [5,6,7,8]   # Change this to specify your desired loops
    ...
    config = BayesConfig(
        ...
        validation_step=[2],  # [1] = l+1 validation, [2] = l+2 validation, [1, 2] = both l+1 and l+2
        test_loop=9  # Optional: evaluate inference model on this loop and save scores (set to None to skip)
    )
