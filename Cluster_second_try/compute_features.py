"""
Simple feature computation and saving.
Run this once to compute all features for a given loop order.
"""
...
def load_graph_edges(loop_order, data_path=None):
    ...
    if data_path is None:
        data_path = f'../Graph_Edge_Data/den_graph_data_{loop_order}.csv'
...
FEATURE_FUNCTIONS = {
    'laplacian_eigenvalues': compute_laplacian_eigenvalues_normalized,
...
def compute_and_save_feature(feature_name, loop_order, chunk_size=1000, n_jobs=4):
    ...
    output_dir = Path(f'../Graph_Edge_Data/features_loop_{loop_order}')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--loop', type=int, required=True, help='Loop order')
    ...
    if args.feature:
        compute_and_save_feature(args.feature, args.loop, args.chunk_size, args.n_jobs)
    else:
        compute_all_features(args.loop, args.chunk_size, args.n_jobs)
